...
TYPOLOGIES: Dict[str, ElectionTypology] = {
    "general_alpha": ElectionTypology(
        key="general_alpha",
        name="General Alpha",
        description=(
...
def list_profiles() -> List[ElectionTypology]:
    return [TYPOLOGIES[k] for k in sorted(TYPOLOGIES.keys())]


def get_profile(key: str) -> ElectionTypology:
    if key not in TYPOLOGIES:
        raise KeyError(f"Unknown profile '{key}'. Available: {', '.join(sorted(TYPOLOGIES.keys()))}")
    return TYPOLOGIES[key]
